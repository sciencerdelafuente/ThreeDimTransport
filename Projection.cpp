/**************************************************************************************/  
//  Computes the the projection of a surface traveling between two given layers
...
long double vector_r,vector_s,absr,abss,angulov,Proj_s;
        ...
        long double factor_numerador,factor_denominador,factor;
    ...
    long double ZZ[2][2];
    long double xf_dx,yf_dx,xf_dy,yf_dy;
   ...
   for(long double Px=min_x;Px<max_x;Px=Px+0.2)
   {
   for(long double Py=min_x;Py<max_x;Py=Py+0.2)
   {
    ...
    Lyapunovpeq=0;
          ...
          contando=0;
          for(long double Ax=Px;Ax<Px+radio;Ax=Ax+dx)
          { 
            for(long double Ay=Py;Ay<radio+Py;Ay=Ay+dx)
            {
         ...
         do{
            ...
            anterior_yn=yn;
            anterior_zn=zn;
            anterior_var_xx=var_xx;
            anterior_var_yx=var_yx;
            anterior_var_zx=var_zx;
            anterior_var_xy=var_xy;
            anterior_var_yy=var_yy;
            anterior_var_zy=var_zy;
            anterior_var_xz=var_xz;
            anterior_var_yz=var_yz;
            anterior_var_zz=var_zz;
            tau_anterior=tau0;
            ...
            posterior_yn=yn;
            posterior_zn=zn;
            posterior_var_xx=var_xx;
            posterior_var_yx=var_yx;
            posterior_var_zx=var_zx;
            posterior_var_xy=var_xy;
            posterior_var_yy=var_yy;
            posterior_var_zy=var_zy;
            posterior_var_xz=var_xz;
            posterior_var_yz=var_yz;
            posterior_var_zz=var_zz;
            tau_posterior=tau0;
          ...
          var_xx=anterior_var_xx+lambda*(posterior_var_xx-anterior_var_xx);
          var_yx=anterior_var_yx+lambda*(posterior_var_yx-anterior_var_yx);
          var_zx=anterior_var_zx+lambda*(posterior_var_zx-anterior_var_zx);
          var_xy=anterior_var_xy+lambda*(posterior_var_xy-anterior_var_xy);
          var_yy=anterior_var_yy+lambda*(posterior_var_yy-anterior_var_yy);
          var_zy=anterior_var_zy+lambda*(posterior_var_zy-anterior_var_zy);
          var_xz=anterior_var_xz+lambda*(posterior_var_xz-anterior_var_xz);
          var_yz=anterior_var_yz+lambda*(posterior_var_yz-anterior_var_yz);
          var_zz=anterior_var_zz+lambda*(posterior_var_zz-anterior_var_zz);
  ...
  if((d[0]>d[1] && d[0]<d[2]) || (d[0]<d[1] && d[0]>d[2]) )
  {
     ll=d[0];
     vec[0]=v[0][0];
     vec[1]=v[1][0];
  ...
  if((d[1]>d[0] && d[1]<d[2]) || (d[1]<d[0] && d[1]>d[2]) )
  {
     ll=d[1];
     vec[0]=v[0][1];
     vec[1]=v[1][1];
  ...
  if((d[2]>d[1] && d[2]<d[0]) || (d[2]<d[1] && d[2]>d[0]) )
  {
     ll=d[2];
     vec[0]=v[0][2];
     vec[1]=v[1][2];
     vec[2]=v[2][2];
      ...
      //ProyecctionM
       ...
       longitud=sqrt((vecM[0]*vecM[0])+(vecM[1]*vecM[1])+(vecM[2]*vecM[2]));
       vecM[0]=(vecM[0]*llM)/longitud;
       vecM[1]=(vecM[1]*llM)/longitud;
       vecM[2]=(vecM[2]*llM)/longitud;

       normal[0]=(vecM[0]*vz-vx*vecM[2]);
       normal[1]=(vecM[1]*vz-vy*vecM[2]);
       normal[2]=0;

       longitud=sqrt((normal[0]*normal[0])+(normal[1]*normal[1])+(normal[2]*normal[2]));

       projectionM=fabs((longitud)/vz);


      //Proyecctionm

       longitud=sqrt((vec[0]*vec[0])+(vec[1]*vec[1])+(vec[2]*vec[2]));
       vec[0]=(vec[0]*ll)/longitud;
       vec[1]=(vec[1]*ll)/longitud;
       vec[2]=(vec[2]*ll)/longitud;

       normal[0]=(vec[0]*vz-vx*vec[2]);
       normal[1]=(vec[1]*vz-vy*vec[2]);
       normal[2]=0;
  ...
  cout<<projectionM<<" "<<llM<<endl;



                   LyapunovM=LyapunovM+projectionM;
                   Lyapunov=Lyapunov+llM;
                   Lyapunovpeq=Lyapunovpeq+ll;
                   Lyapunovm=Lyapunovm+projectionm;
                   Taumedia=Taumedia+T;
       ...
       LyapunovM=LyapunovM/(contando);
       Lyapunov=Lyapunov/(contando);
       Lyapunovm=Lyapunovm/(contando);
       Lyapunovpeq=Lyapunovpeq/(contando);
       Taumedia=Taumedia/contando;
          ...
          for(int fff=0;fff<number_boxes;fff++)
          {
              if(L[fff]!=0)
              {
               ...
               L[fff]=L[fff]/contando;
              
              if(L[fff]!=0)
              {
                 entropia=entropia-((L[fff]*log(L[fff])));
              }
          }



  // cout<<LyapunovM<<" "<<Lyapunov<<"    "<<exp(entropia)<<endl;
