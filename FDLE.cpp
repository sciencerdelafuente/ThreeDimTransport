/**************************************************************************************/  
//  Computes the two-layer map of particles and the FDLE from the analytical ABC flow 
...
long double formula;
        ...
        long double cubex[total_particles],cubey[total_particles];
        long double cubex0[total_particles],cubey0[total_particles];
    ...
    Laux=(double *)malloc(number_boxes*sizeof(double*));
     ...
     double *rectangulo;
    rectangulo=(double *)malloc(number_boxes*sizeof(double*));
   ...
   long double dx2=(radio*0.5)/particles_per_box;
   long double altura,A1,A2;
   long double perturbation=0.00001;

   long double wxM,wxm,wyM,wym;
   long double Px=0;
   long double Py=5.6;
  ...
  for(long double Px=min_x;Px<max_x;Px=Px+dx_boxes)
   {
   for(long double Py=0;Py<max_x;Py=Py+dx_boxes)
   {
       ...
       vecM[0]=((xf_dy-xf_dx)/perturbation);
       vecM[1]=((yf_dy-yf_dx)/perturbation);

       vec[0]=((xf_dxy-xf)/perturbation);
       vec[1]=((yf_dxy-yf)/perturbation);

      modulo1=sqrt(vecM[0]*vecM[0]+vecM[1]*vecM[1]);
      modulo2=sqrt(vec[0]*vec[0]+vec[1]*vec[1]);

        ll=modulo1;
        if(modulo2>modulo1){ll=modulo2;}
 ...
 Lyapunov=llm;
 if(Lyapunov<1){Lyapunov=1;}
 Lyapunov=Lyapunov*llM;

  

  cout<<Px<<" "<<Py<<" "<<Pz<<" "<<xn<<" "<<yn<<" "<<layer2<<" "<<llM<<" "<<llm<<" "<<llM*llm<<" "<<log(llM)/T<<" "<<log(llm)/T<<" "<<(log(llM)/T)+(log(llm)/T)<<" "<<Lyapunov<<endl;
